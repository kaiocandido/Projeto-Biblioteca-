  ...
  <main class="p-4 max-w-5xl mx-auto mt-6 bg-gray-800 rounded-lg shadow-lg relative">
    ...
    <div class="flex justify-between mb-4">
      ...
      <div class="flex gap-2">
        <a href="cadastro_livro.php?id=<?= $id_livro ?? ''?>" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1 px-4 rounded text-sm">
          Alterar
        </a>
        <a href="excluir_livro.php?id=<?= $id_livro ?? '' ?>" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-1 px-4 rounded text-sm">
          Excluir
        </a>
      </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="flex-shrink-0">
        <img src="../imagens/<?=  $imagem ?? '' ?>" alt="Capa do Livro" class="w-full lg:w-60 h-auto rounded-lg shadow-md" />
      </div>
      ...
      <div class="flex-1">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p class="text-white font-medium"><?= $nome_autor ?? '' ?></p>
          </div>
          <div>
            <span class="text-gray-400">Editora:</span>
            <p class="text-white font-medium"><?= $nome_editora ?? '' ?></p>
          </div>
          <div>
            <span class="text-gray-400">Data de Cadastro:</span>
            <p class="text-white font-medium"><?= $data_cadastro ?? '' ?></p>
          </div>
          <div>
            <span class="text-gray-400">Categoria:</span>
            <p class="text-white font-medium"><?= $categoria ?? '' ?></p>
          </div>
          <div>
            <span class="text-gray-400">ISBN:</span>
              <p class="text-white font-medium"><?= $ISBN ?? '' ?></p>
            </div>
          <div>
            <span class="text-gray-400">Status:</span>
            <p class="text-white font-medium"><?= $status_desc ?? '' ?></p>
          </div>
          <div>
            <span class="text-gray-400">Ano de Publicação:</span>
            <p class="text-white font-medium"><?= $ano_publicacao ?? '' ?></p>
          </div>
          <div>
    ...
    <!-- Lista de Exemplares -->
        <section class="mt-6 bg-gray-700 p-4 rounded-lg">
          <div class="flex justify-between items-center mb-4">
            ...
            <a href="cadastro_exemplar.php?id_livro=<?= $id_livro ?? '' ?>" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded">
              Novo
            </a>
          </div>
          <table class="min-w-full table-auto text-sm">
            ...
            <tbody>
              <?php
                foreach($dadosExemplaresc as $ex){
              ?>
                  <tr class="hover:bg-gray-600">
                    <td class="py-2 px-4"><?= $ex['codigo'] ?? '' ?></td>
                    <td class="py-2 px-4"><?= $ex['data_cadastro'] ?? '' ?></td>
                    <td class="py-2 px-4"><?= $ex['status_exemplar'] ?? '' ?></td>
                    <td class="py-2 px-4">
                      <a href="excluir_exemplar.php?id=<?= $ex['id_exemplar'] ?>" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-4 rounded">Excluir</a>
                    </td>
                  </tr>
              <?php
              }
